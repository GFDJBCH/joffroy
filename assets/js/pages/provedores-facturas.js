...
let current_socio = null;
if (url.searchParams.has("identificador")) {
    const identificador = url.searchParams.get("identificador").trim();
    if (identificador !== "") {
        current_socio = identificador;
        cargarDatos();
    } else {
...
const lblFechaRegistro = document.getElementById("lbl_fecha_registro");
const lblNombreProveedor = document.getElementById("lbl_nombre_proveedor");
const lblNombreFiscal = document.getElementById("lbl_nombre_fiscal");
const lblProveedorDireccion = document.getElementById("lbl_proveedor_direccion");
const proveedorProgress = document.getElementById("proveedor-progress");
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.length > 0) {
                ...
                lblNombreProveedor.innerHTML = `${prs_nombre} ${prs_apellidos} ${activo === '1' ? '<i class="bi bi-person-check text-success"></i>' : ''}`;
                lblNombreFiscal.innerHTML = `<strong>${tax_id}</strong>/${(nbr_negocio === '' ? nbr_comercial : nbr_negocio)}`;
                lblProveedorDireccion.innerHTML = `${calle} ${nmr_externo} ${auxInterno}, ${colonia} ${cdg_postal}`;
                lblFechaRegistro.innerText = fch_creacion;
                ...
                isForeign = parseInt(tp_proveedor) === 3;
...
var mdlInvoiceForeignElement = document.getElementById('mdlInvoiceForeign')
        ...
        billValidator.validate().then(function (status) {
            if (status === 'Valid') {
                ...
                setTimeout(function () {
                    ...
                    billData.append("partner", current_socio);
...
document.addEventListener("DOMContentLoaded", function () {
    let timeoutId;
    previewOrderNumber.addEventListener("input", function () {
        clearTimeout(timeoutId);
        timeoutId = setTimeout(function () {
            const valor = previewOrderNumber.value;
            if (valor.trim() !== "") {
                guardarInformacion(valor);
            }
        }, 2000);
    });
    previewOrderNumber.addEventListener("keydown", function (event) {
        if (event.key === "Enter") {
            const valor = previewOrderNumber.value;
            if (valor.trim() !== "") {
                guardarInformacion(valor);
    ...
    /*previewOrderNumber.addEventListener("blur", function() {
        const valor = previewOrderNumber.value;
        if (valor.trim() !== "") {
            guardarInformacion(valor);
        }
    });*/
    function guardarInformacion(valor) {
        const dataIdValue = previewOrderNumber.getAttribute("data-id");

        const billData = new FormData();
        billData.append("invoice", dataIdValue);
        billData.append("orderNumber", valor);

        fetch('controller/guardar-factura-numero.php', {
            method: "POST",
            body: billData
        })
            .then(response => response.json())
...
<a href="${dataPDF.path}" target="_blank" class="btn btn-outline btn-outline-dashed btn-outline-default p-7 d-flex align-items-center mb-5">
    ...
    previewOrderNumber.setAttribute("data-id", data.id);

    previewOrderNumber.value = data.orderNumber;
...
function parseXML(xmlString) {
    ...
    for (var i = 0; i < conceptoElements.length; i++) {
        ...
        for (var j = 0; j < trasladoElements.length; j++) {
            var traslado = trasladoElements[j];
    ...
    for (var k = 0; k < trasladoElementsGlobal.length; k++) {
        var trasladoGlobal = trasladoElementsGlobal[k];
    ...
    if (partner_data.dias_de_credito !== null && partner_data.dias_de_credito !== 0) {
...
previewOrderNumber.placeholder = language === "en" ? "Order number" : "Número de orden";
...
document.querySelector(`label[for="${previewOrderNumber.id}"]`).textContent = language === "en" ? "Order number" : "Número de orden";
